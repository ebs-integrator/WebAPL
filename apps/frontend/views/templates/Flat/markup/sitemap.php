

<section>
    <div class="wrap">
        <div class="left_site">
            <p class="s_title">1. CETĂȚENI ȘI BUSINESS:</p>
        </div>
        <div class="right_block">
            <div class='sitemap'>
                <p><a href='javascript:;'>1.1.   Beneficii sociale și ajutoare</a></p>
                <ul>
                    <li><a href='javascript:;'>Ajutoare sociale familiale</a></li>
                    <li><a href='javascript:;'>Bursa muncii / Ajutorul de șomaj</a></li>
                    <li><a href='javascript:;'>Pensionarea și ajutorul social</a></li>
                    <li><a href='javascript:;'>Consiliere socială</a></li>
                    <li><a href='javascript:;'>Puncte medicale locale</a></li>
                    <li><a href='javascript:;'>Decesul și ajutorul de deces</a></li>
                    <li><a href='javascript:;'>Adăposturi sociale și case de bătrâni</a></li>
                    <li><a href='javascript:;'>Violența domestică / Cazuri de urgență</a></li>
                    <li><a href='javascript:;'>Raportează o fraudă socială</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>
            <div class='sitemap'>
                <p><a href='javascript:;'>1.2.   Taxe și impozite - titilu meniu</a></p>
                <ul>
                    <li><a href='javascript:;'>Impozite și taxe locale</a></li>
                    <li><a href='javascript:;'>Taxa pe amenajarea teritoriului</a></li>
                    <li><a href='javascript:;'>Taxa de publicitate</a></li>
                    <li><a href='javascript:;'>Taxa de organizare a licitațiilor / loteriilor</a></li>
                    <li><a href='javascript:;'>Taxa pentru unități comerciale</a></li>
                    <li><a href='javascript:;'>Taxa de piața</a></li>
                    <li><a href='javascript:;'>Taxa de cazare</a></li>
                    <li><a href='javascript:;'>Taxa de parcare</a></li>
                    <li><a href='javascript:;'>Taxa pentru unități stradale</a></li>
                    <li><a href='javascript:;'>Taxa pentru evacuarea deșeurilor</a></li>
                    <li><a href='javascript:;'>Impozitul pe bunuri imobiliare</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>
            <div class='sitemap'>
                <p><a href='javascript:;'>1.3.   Business și Antreprenoriat</a></p>
                <ul>
                    <li><a href='javascript:;'>Investește în orașul Strășeni</a></li>
                    <li><a href='javascript:;'>Cum să înregistrezi o societate comercială</a></li>
                    <li><a href='javascript:;'>Cum să înregistrezi o gospodărie de fermieri</a></li>
                    <li><a href='javascript:;'>Cum să înregistrezi o întreprindere individuală</a></li>
                    <li><a href='javascript:;'>Incubatoare de afaceri și companii de consultanță</a></li>

                </ul>
            </div>
            <div class='sitemap'>
                <p><a href='javascript:;'>1.4.   Autorizații și certificate</a></p>
                <ul>
                    <li><a href='javascript:;'>Autorizația pentru unități comerciale</a></li>
                    <li><a href='javascript:;'>Autorizația de construcție</a></li>
                    <li><a href='javascript:;'>Autorizația de desființare</a></li>
                    <li><a href='javascript:;'>Certificatul de urbanism pentru proiectare</a></li>
                    <li><a href='javascript:;'>Certificat privind lipsa / existența restanțelor</a></li>
                    <li><a href='javascript:;'>Certificat privind componenta familiei</a></li>
                    <li><a href='javascript:;'>Certificate necesare pentru construcția casei sau a unui obiectiv comercial</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>

            <div class='clearfix'></div>
        </div>
        <div class='clearfix'></div>
        <div class="left_site">
            <p class="s_title">2. PRIMĂRIA</p>
        </div>
        <div class="right_block">
            <div class='sitemap'>
                <p><a href='javascript:;'>2.1.   Primăria</a></p>
                <ul>
                    <li><a href='javascript:;'>Primarul</a></li>
                    <li><a href='javascript:;'>Bursa muncii / Ajutorul de șomaj</a></li>
                    <li><a href='javascript:;'>Pensionarea și ajutorul social</a></li>
                    <li><a href='javascript:;'>Consiliere socială</a></li>
                    <li><a href='javascript:;'>Puncte medicale locale</a></li>
                    <li><a href='javascript:;'>Decesul și ajutorul de deces</a></li>
                    <li><a href='javascript:;'>Adăposturi sociale și case de bătrâni</a></li>
                    <li><a href='javascript:;'>Violența domestică / Cazuri de urgență</a></li>
                    <li><a href='javascript:;'>Raportează o fraudă socială</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>
            <div class='sitemap'>
                <p><a href='javascript:;'>2.2.   Regulamente, rapoarte și planuri</a></p>
                <ul>
                    <li><a href='javascript:;'>Impozite și taxe locale</a></li>
                    <li><a href='javascript:;'>Taxa pe amenajarea teritoriului</a></li>
                    <li><a href='javascript:;'>Taxa de publicitate</a></li>
                    <li><a href='javascript:;'>Taxa de organizare a licitațiilor / loteriilor</a></li>
                    <li><a href='javascript:;'>Taxa pentru unități comerciale</a></li>
                    <li><a href='javascript:;'>Taxa de piața</a></li>
                    <li><a href='javascript:;'>Taxa de cazare</a></li>
                    <li><a href='javascript:;'>Taxa de parcare</a></li>
                    <li><a href='javascript:;'>Taxa pentru unități stradale</a></li>
                    <li><a href='javascript:;'>Taxa pentru evacuarea deșeurilor</a></li>
                    <li><a href='javascript:;'>Impozitul pe bunuri imobiliare</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>
            <div class='sitemap'>
                <p><a href='javascript:;'>2.3.   Întreprinderi subordonate- titilu meniu</a></p>
                <ul>
                    <li><a href='javascript:;'>Autorizația pentru unități comerciale</a></li>
                    <li><a href='javascript:;'>Autorizația de construcție</a></li>
                    <li><a href='javascript:;'>Autorizația de desființare</a></li>
                    <li><a href='javascript:;'>Certificatul de urbanism pentru proiectare</a></li>
                    <li><a href='javascript:;'>Certificat privind lipsa / existența restanțelor</a></li>
                    <li><a href='javascript:;'>Certificat privind componenta familiei</a></li>
                    <li><a href='javascript:;'>Certificate necesare pentru construcția casei sau a unui obiectiv comercial</a></li>
                    <li><a href='javascript:;'>Întrebări frecvente / Modele de cereri / Ghid de completare</a>
                        <ul>
                            <li><a href='javascript:;'>Întrebări frecvente</a></li>
                            <li><a href='javascript:;'>Modele de cereri</a></li>
                            <li><a href='javascript:;'>Ghid de completare a cererii</a>
                                <ul>
                                    <li><a href='javascript:;'>Ghid de completare a cererii de ajutor social</a></li>
                                    <li><a href='javascript:;'>Ghid de completare a cererii ajutor de somaj</a></li>
                                </ul>

                            </li>
                        </ul>

                    </li>

                </ul>
            </div>

        </div>
    </div>

</section>




