...
if (!function_exists('dinamic_field')) {
     ...
     * @return mixed
     */
    function dinamic_field($field, $entry, $lang_dependent, $points = array()) {
        if (array_key_exists('{value}', $points) && $field->value_filter) {
            if (method_exists('DinamicFields', $field->value_filter))
                $points['{value}'] = call_user_func(array('DinamicFields', $field->value_filter), $points['{value}'], $entry, $lang_dependent, $field);
        }
...
if (!function_exists('urigen')) {

    /**
     * Prepare uri
     * @param string $str
     * @param array $replace
     * @param chars $delimiter
     * @return string
     */
    function urigen($str, $replace = array(), $delimiter = '-') {
